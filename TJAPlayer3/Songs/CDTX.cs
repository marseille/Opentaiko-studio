...
//		public bool bBASS演奏で直前のBASSを消音する;
        ...
        */
        ...
        public int get_movieoffset()
        {
            return this.nMOVIEOFFSET;
        }
                ...
                if (!string.IsNullOrEmpty(strCommandParam))
                {
                    ...
                    for (int index = 0; index < strFiles.Length; index++)
                    {
                        ...
                        if (File.Exists(strFilePath[index]))
                        {
                            try
                            {   
                                //Only reload if inside training mode
                                if (TJAPlayer3.ConfigIni.bTokkunMode == true || TJAPlayer3.r現在のステージ.eステージID == CStage.Eステージ.曲読み込み)//起動時に重たくなってしまう問題の修正用
